...
const copy = {
  es: {
    heroEyebrow: "Signal Pack / Brutalist",
    heroTitle: "Diseno con impacto directo.",
    heroText:
      "Esta tercera direccion no busca verse prolija: busca ser memorable. Tipografia brutal, color fuerte y estructura de manifiesto tecnico.",
    ...
    talks: "Charlas",
    skills: "Tecnologias",
    proofTitle: "Pruebas de ejecucion",
    proofSubtitle: "No es solo estetica: los datos del YAML se transforman en senales claras para tomar decision.",
    proofQuote: "Backend como columna, QA como garantia, Unity como expansion.",
    servicesTitle: "Bloques de servicio",
    servicesSubtitle: "Cada bloque es una capacidad concreta, no humo comercial.",
    experienceTitle: "Ruta profesional",
    experienceSubtitle: "Empresas, periodos y logros clave en lectura rapida.",
    projectsTitle: "Proyectos seleccionados",
    projectsSubtitle: "Casos reales para validar profundidad tecnica y delivery.",
    recognitionTitle: "Charlas y credenciales",
    recognitionSubtitle: "Autoridad tecnica sin inflar la pagina de texto.",
    contactTitle: "Si te gusto esta tercera direccion, la iteramos",
    contactSubtitle: "Podemos empujarla aun mas y convertirla en tu portada principal.",
    email: "Email",
    linkedin: "LinkedIn",
    github: "GitHub",
    portfolio: "Portfolio",
  ...
  en: {
    heroEyebrow: "Signal Pack / Brutalist",
    heroTitle: "Design with direct impact.",
    heroText:
      "This third direction is not trying to look polite: it is trying to be memorable. Brutal typography, loud color, and a technical manifesto structure.",
    ...
    talks: "Talks",
    skills: "Technologies",
    proofTitle: "Execution signals",
    proofSubtitle: "Not only aesthetics: YAML data becomes clear decision signals.",
    proofQuote: "Backend as backbone, QA as guarantee, Unity as expansion.",
    servicesTitle: "Service blocks",
    servicesSubtitle: "Each block is a concrete capability, not marketing noise.",
    experienceTitle: "Professional route",
    experienceSubtitle: "Companies, periods, and key achievements in fast reading.",
    projectsTitle: "Selected projects",
    projectsSubtitle: "Real cases that validate technical depth and delivery.",
    recognitionTitle: "Talks and credentials",
    recognitionSubtitle: "Technical authority without turning the page into a wall of text.",
    contactTitle: "If you like this third direction, we can iterate it",
    contactSubtitle: "We can push it even further and make it your main cover.",
    email: "Email",
    linkedin: "LinkedIn",
    github: "GitHub",
    portfolio: "Portfolio",
...
function buildProof() {
  ...
  el.proof.innerHTML = `
    ...
    <div class="metric-panel reveal">
      ...
      <div class="chip-row" style="margin-top:0.65rem;">${companies.map((name) => `<span class="badge">${escapeHtml(name)}</span>`).join("")}</div>
    </div>`;
}
...
function buildServices() {
  ...
  const cards = services.slice(0, 6).map((service, index) => {
    const colors = ["var(--yellow)", "var(--cyan)", "var(--pink)", "var(--green)", "var(--orange)", "#d6d6ff"];
    return `
      <article class="card reveal" style="background:${colors[index % colors.length]};">
  ...
  const cards = experience.slice(0, 6).map((item) => {
    ...
    return `
      <article class="card reveal">
        ...
        <div class="chip-row">${roles.map((role) => `<span class="role">${escapeHtml(role)}</span>`).join("")}</div>
...
function buildContact() {
  ...
  const actions = [
    personal.email ? `<a class="btn btn--primary" href="mailto:${escapeHtml(personal.email)}">${iconMarkup("mail")}<span>${escapeHtml(tx("email"))}</span></a>` : "",
    links.linkedin ? `<a class="btn btn--line" href="${escapeHtml(links.linkedin)}" target="_blank" rel="noopener noreferrer">${iconMarkup("linkedin")}<span>${escapeHtml(tx("linkedin"))}</span></a>` : "",
    links.github ? `<a class="btn btn--line" href="${escapeHtml(links.github)}" target="_blank" rel="noopener noreferrer">${iconMarkup("github")}<span>${escapeHtml(tx("github"))}</span></a>` : "",
    links.portfolio ? `<a class="btn btn--line" href="${escapeHtml(links.portfolio)}" target="_blank" rel="noopener noreferrer">${iconMarkup("portfolio")}<span>${escapeHtml(tx("portfolio"))}</span></a>` : "",
  ].filter(Boolean);
